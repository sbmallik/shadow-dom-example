...
def run_tests(platform, browser, version, junit_dir)
  sh("platform=\"#{platform}\" browserName=\"#{browser}\" version=\"#{version}\" parallel_cucumber features -o \"--format junit --out #{junit_dir} --format pretty\" -n 20 #{ENV['test_options']}") do | success, _exit_code |
    @success &= success
  end
end
...
task :windows_10_firefox_47 do
  run_tests('Windows 10', 'firefox', '47.0', 'junit_reports/windows_10_firefox_47')
end
